...
foam.CLASS({
  ...
  methods: [
    ...
    function validate() {
      this.SUPER();
      foam.assert(this.size > 0,
                  'ClockCache: Size is <= 0');
      foam.assert(this.numItems_ >= 0,
                  'ClockCache: Number of items is < 0');
      foam.assert(this.numItems_ <= this.size,
                  'ClockCache: Number of items exceeds size');
    },
    function isFull() {
      this.validate();

      return this.numItems_ === this.size;
    },
    function store(data) {
      this.validate();
      ...
      if (this.isFull()) {
        this.evictOne_();
        idx = this.clockIdx_;
        this.clockIdx_ = (idx + 1) % this.size;
      } else {
        idx = this.numItems_;
        this.numItems_++;
