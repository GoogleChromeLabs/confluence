...
foam.CLASS({
  ...
  properties: [
    ...
    {
      name: 'valueName',
      documentation: 'The property name where y-axis value are in each object',
      required: true,
      final: true,
    },
    {
  ...
  methods: [
    {
      ...
      let rightAxisUpperBound = Math.ceil(d3.max(
        this.metricSeries, (apiNumArrary) => {
          return d3.max(apiNumArrary.values, (d) => d[this.valueName]);
        }) / 20) * 20;
      let rightAxisLowerBound = Math.floor(d3.min(
        this.metricSeries, (apiNumArrary) => {
          return d3.min(apiNumArrary.values, (d) => d[this.valueName]);
        }) / 20) * 20;
      ...
      svg.selectAll('dot')
        ...
        .enter().append('circle')
          ...
          .attr('cy', (d) => y(d[this.valueName]) + margin.top)
      ...
      browser.append('text')
        ...
        .attr('transform', (d) =>
          `translate(${x(d.value.date)},${y(d.value[this.valueName])})`)
        .attr('x', 3)
        .attr('dy', '0.35em')
        .style('font', '10px')
        .text((d) => d.id);
