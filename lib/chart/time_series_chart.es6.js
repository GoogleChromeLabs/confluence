  ...
  methods: [
    {
      ...
      code: function() {
        ...
        const rightAxisUpperBound = Math.ceil(d3.max(this.metricSeries,
            (apiNumArrary) => {
              return d3.max(apiNumArrary.values, (d) => d.value);
            }) / 20) * 20;
        const rightAxisLowerBound = 0;
        // Since the date for each release array is the same.
        const dates = this.metricSeries[0].values.map((d) => d.date);
        ...
        const release = g.selectAll('.release')
