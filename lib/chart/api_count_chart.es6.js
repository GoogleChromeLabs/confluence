...
function apiCountChartController($scope, $timeout) {
  const createNewDiffView = function(minuend, subtrahend) {
  ...
  this.$postLink = function() {
    ...
    $timeout(function() {
      ...
      const div = d3.select(`#${this.chartId}`).append('div');
      ...
      const rightAxisUpperBound = Math.ceil(d3.max(
          this.apiCount, (d) => d.totalApis + d.removedApis) / 500) * 500;
      const rightAxisLowerBound = Math.floor(d3.min(
          this.apiCount, (d) => d.totalApis - d.newApis) / 500) * 500;
      // X axis is release date.
      const x = d3.scaleTime()
          .rangeRound([0, width])
          .domain(d3.extent(this.apiCount, (d) => d.releaseDate));
      ...
      const tooltips = d3.select(`#${this.chartId}-tooltip`);
      ...
      const areaRemovedAPI = d3.area()
          .x((d) => x(d.releaseDate))
          .y1((d) => y(d.totalApis + d.removedApis))
          .y0((d) => y(d.totalApis));
      // Number of new APIs.
      const areaNewAPI = d3.area()
          .x((d) => x(d.releaseDate))
          .y1((d) => y(d.totalApis))
...
angular.module('confluence').component('apiCountChart', {
  template: require('../../static/component/chart.html'),
  controller: ['$scope', '$timeout', apiCountChartController],
  bindings: {
    apiCount: '<',
    title: '<',
    chartId: '<',
