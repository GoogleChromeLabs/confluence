...
function vendorSpecificChartController($scope, $timeout) {
  ...
  this.$onInit = function() {
    ...
    $timeout(function() {
      let browserData = Object.keys(this.vendorSpecificMetric)
        .map((browserName) => {
          return {
            browserName,
            values: this.vendorSpecificMetric[browserName],
          };
        });
      let width = 720;
      let height = 360;
      ...
      let rightAxisUpperBound = Math.ceil(d3.max(
        browserData, (apiNumArrary) => {
          return d3.max(apiNumArrary.values, (d) => d.numVendorSpecific);
        }) / 20) * 20;
      let rightAxisLowerBound = Math.floor(d3.min(
        browserData, (apiNumArrary) => {
          return d3.min(apiNumArrary.values, (d) => d.numVendorSpecific);
        }) / 20) * 20;
      // Since the date for each browser array is the same.
      let dates = this.vendorSpecificMetric[Object
        .keys(this.vendorSpecificMetric)[0]].map((d) => d.date);
      ...
      let z = d3.scaleOrdinal(d3.schemeCategory10)
        .domain(Object.keys(this.vendorSpecificMetric));
      // Draw aixises.
      g.append('g')
      ...
      svg.selectAll('dot')
        ...
        .enter().append('circle')
          ...
          .attr('cy', (d) => y(d.numVendorSpecific) + margin.top)
      ...
      browser.append('text')
        ...
        .attr('transform', (d) =>
          `translate(${x(d.value.date)},${y(d.value.numVendorSpecific)})`)
        .attr('x', 3)
        .attr('dy', '0.35em')
        .style('font', '10px')
        .text((d) => d.id);
