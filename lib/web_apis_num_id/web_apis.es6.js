...
foam.CLASS({
  ...
  package: 'com.web.api',
  requires: [
    'com.web.api.Browser_numId',
    'com.web.api.BrowserAPI_numId',
    'com.web.api.WebInterface_numId',
    'foam.dao.EasyDAO',
  ...
  properties: [
    {
      ...
      factory: function() {
        return this.EasyDAO.create({
          name: 'BrowserAPIDAO',
          of: this.BrowserAPI_numId,
          daoType: 'MDAO',
        }).addPropertyIndex(this.BrowserAPI_numId.INTERFACE_ID);
      },
    },
    {
      name: 'browsers',
  ...
  methods: [
    {
      ...
      code: function(browserName, browserVersion,
        ...
        let browser = this.Browser_numId.create({
        ...
        for (let i = 0; i < interfaceNames.length; i += 1) {
          ...
          for (let j = 0; j < apiCatalog[interfaceName].length; j += 1) {
            ...
            let webInterface = this.WebInterface_numId.create({
              interfaceName,
              APIName,
            });
            let interfaceId = webInterface.id;
            this.interfaces.put(webInterface);
            this.browserAPIs.put(this.BrowserAPI_numId.create({
              browserId,
              interfaceId,
    ...
    },
    {
      ...
      code: function(browserKey) {
        ...
        return this.Browser_numId.create({
        ...
        return new Promise(function(resolve) {
          ...
          for (let i = 0; i < browserAPIs.length; i += 1) {
            ...
            promises.push(this.interfaces
              .where(this.mlang.EQ(this.WebInterface_numId.ID, interfaceId)).select()
              .then((arrayDao) => {
          ...
          Promise.all(promises).then(function() {
            console.timeEnd('num-id-process');
            resolve(apiMap);
          });
        }.bind(this));
      },
    },
    {
      ...
      code: function(browserKeys) {
        return new Promise(function(resolve) {
          ...
          console.time('num-id-query');
          this.browserAPIs
            .where(this.mlang.IN(this.BrowserAPI_numId.BROWSER_ID, browserIds))
            .select(this.GroupBy.create({
              arg1: this.BrowserAPI_numId.INTERFACE_ID,
              arg2: this.ArrayDAO.create(),
            }))
            .then(function(groups) {
              console.timeEnd('num-id-query');
              console.time('num-id-process');
              let groupKeys = groups.groupKeys;
              let apiMap = {};
              let browserDict = {}; // O(1) lookup for browser's index in header.
              apiMap._header = browserKeys.slice();
              let numBrowsers = apiMap._header.length;
              ...
              for (let i = 0; i < groupKeys.length; i++) {
                ...
                promises.push(this.interfaces
                  ...
                  for (let j = 0; j < groups.groups[interfaceId].array.length; j++) {
                    apiMap[interfaceName][APIName][
                      browserDict[groups.groups[interfaceId].array[j].browserId]] = true;
                  }
                }));
              };
              Promise.all(promises).then(function() {
