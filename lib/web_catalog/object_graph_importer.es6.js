  ...
  methods: [
    {
      ...
      documentation: `Reads object graph files from objectGraphPath
          ...
          using apiImporter.`,
      code: function() {
        const ogFileRegExp = /window_[a-zA-Z0-9_.]+_[a-zA-Z0-9_.]+_[a-zA-Z0-9_.]+_[a-zA-Z0-9_.]+[.]json$/;
        let promises = [];
        ...
        for (let i = 0; i < objectGraphFiles.length; i++) {
          ...
          if (stat.isFile()) {
            ...
            let releaseInfo = this.getReleaseInfo(objectGraphFiles[i]);

            this.info(`Extracting APIs from
    ...
    function getReleaseInfo(filename) {
      const json = require(`${this.objectGraphPath}/${filename}`);
      if (json.environment && json.environment.browser &&
        ...
        return this.getReleaseInfoFromJSON_(json);
      } else {
        return this.getReleaseInfoFromFilename_(filename);
      }
    },
    function getReleaseInfoFromJSON_(json) {
      return json.environment;
    },
    function getReleaseInfoFromFilename_(filename) {
