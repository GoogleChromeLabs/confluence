...
foam.CLASS({
  ...
  methods: [
      ...
      code: function() {
        ...
        for (const name of globalNames) {
          ...
          if ((!og.isType(id)) && (!this.isFunctionLike_(id))) {
            ...
            this.libs_[libId] = this.dedupConcat_((this.libs_[libId] || []),
                                                 names);
          }
        }
    ...
    function getAllProtosAPIs_(id) {
      const og = this.objectGraph;
      let lowerProtoId = og.getPrototype(id);
      let apis = [];
      while (!og.isType(lowerProtoId)) {
        apis = this.dedupConcat_(apis, this.getClassAPIs_(lowerProtoId));
    ...
    function dedupConcat_(arr1, arr2) {
