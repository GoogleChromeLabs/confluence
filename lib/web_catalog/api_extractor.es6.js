...
foam.CLASS({
  ...
  properties: [
    ...
    },
    {
      ...
      factory: function() {
        return [
          ...
          'a',
          // React DevTools extension installed on BrowserStack
    ...
    },
    {
      ...
      name: 'objectGraphApiMapper',
      factory: function() {
        return this.NullObjectGraphApiMapper.create();
      },
    },
    {
      class: 'FObjectArray',
        ...
        return [
          ...
          this.RemoveInterfacesProcessor.create({
            otherURLs: [
              ...
              // For interface name "__REACT_DEVTOOLS_GLOBAL_HOOK__":
              'https://github.com/GoogleChromeLabs/confluence/issues/463#issuecomment-522119370',
            ],
            interfaceNames: this.blacklistInterfaces,
          }),
          this.RemoveApisProcessor.create({
            interfaceNames: ['Window'],
            apiNameRegExp: /^__REACT_DEVTOOLS_GLOBAL_HOOK__$/,
          }),
          this.RemoveApisProcessor.create({
  ...
  methods: [
      ...
      code: function() {
        ...
        for (const name of globalNames) {
          ...
          if ((!og.isType(id)) && (!this.isFunctionLike_(id))) {
            const libId = id;
            const names = this.getClassNames_(libId);
    ...
    function getFunctionNames_(id) {
      ...
      if (this.functionNamesFromGraphPaths) {
        ...
        const simpleNames = keyParts.map((parts) => parts[parts.length - 1])
            .filter((name) => !this.isBuiltinName_(name))
      ...
      const nameFromOG = og.getFunctionName(id);
    ...
    function getCtorNamesFromProto_(protoId) {
      const og = this.objectGraph;
      const names = this.classNamesFromGraphPaths ? og.getKeys(protoId)
      ...
      if (this.classNamesFromToString) {
        const nameFromToString = this.getCtorNameFromToString_(protoId);
    ...
    function isCtorLike_(id) {
      const og = this.objectGraph;
      return (!og.isType(id)) && og.getObjectKeys(id).includes('prototype');
    },
    function isFunctionLike_(id) {
      if (this.isCtorLike_(id)) return true;
      const og = this.objectGraph;
      const objectKeys = og.getObjectKeys(id);
      return (!og.isType(id)) && objectKeys.includes('arguments') &&
          objectKeys.includes('length');
    },
    function storeAPIsFromCtorOrLib_(ctorId) {
      const apis = this.apis_[ctorId] || [];
    ...
    function getClassAPIs_(id) {
      ...
      if (id === this.function_ || id === this.functionDotPrototype_) {
        ...
        return og.getObjectKeys(id).filter((name) => !this.isBuiltinName_(name));
      }
      // Default: Exclude:
      // (1) built-in object properties,
      // (2) integers,
    ...
    function getInstanceAPIs_(id) {
      const og = this.objectGraph;
      // Exclude built-in object properties and integers.
      return og.getObjectKeys(id)
          .filter((name) => !this.isBuiltinName_(name))
          .filter((name) => !/^[0-9]+$/.test(name));
    },
    ...
    function isBuiltinName_(name) {
      const rewritten = this.objectGraph.nameRewriter.rewriteName(name);
