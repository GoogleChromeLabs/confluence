  ...
  methods: [
    ...
    },
    {
      ...
      code: function(release) {
        this.info(`Computing for single release ${release.id}`);
        return this.recontextualize(release).interfaces.dao.select(this.COUNT())
            .then(count => {
              this.info(`Computed for single release ${release.id}`);
    ...
    },
    {
      ...
      code: function(prev, next) {
        ...
        this.info(`Computing for release pair ${prevInCtx.id}, ${nextInCtx.id}`);
        return Promise.all([
          // [0]: API count.
          ...
          this.info(`Computed for release pair ${prevInCtx.id}, ${nextInCtx.id}`);
          return this.apiVelocityDAO.put(this.ApiVelocityData.create({
