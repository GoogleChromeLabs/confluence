...
foam.CLASS({
  ...
  methods: [
    {
      name: 'compute',
      documentation: `Compute browser-specific API value for "date", given latest release from
          each browser as of "date": "releases".`,
      code: function(releases, date) {
        ...
        for (let i = 0; i < releases.length; i++) {
          ...
          //     release.
          computeForReleases.push(this.releaseDAO.where(this.mlang.OR(
              this.mlang.AND(
                  this.mlang.IN(this.Release.RELEASE_KEY, releaseKeys)),
              this.mlang.AND(
                  this.mlang.EQ(this.Release.BROWSER_NAME, release.browserName),
                  this.mlang.EQ(this.Release.OS_NAME, release.osName),
                  this.mlang.GT(this.Release.RELEASE_DATE, previousYear),
                  this.mlang.LT(this.Release.RELEASE_DATE, date)),
              this.mlang.AND(
                  this.mlang.GT(this.Release.RELEASE_DATE, previousYear),
                  this.mlang.LT(this.Release.RELEASE_DATE, date))))
              .select().then((result) => {
