  ...
  methods: [
    ...
    },
    {
      ...
      code: function(date, browser, current, previous) {
        ...
        // intersection-of-current-releases, use
        // first-current-release.select(INTERSECT(rest-of-current-releases)).
        return firstCurrent.interfaces.select(
            // Intersect with interfaces of other browers in "current" to get
            // interfaces that appear in all "current" browsers.
            this.mlang.INTERSECT(
                currentLessOne.map(brwsr => brwsr.interfaces),
                // Intersect with removed-before-grace-period, not-added-back:
                // {...} INTERSECT (allAPIsOlderThanPostRemoval \
                //                  postRemovalAPIs \ browserAPIs).
                this.mlang.INTERSECT(
