...
foam.CLASS({
  ...
  properties: [
    {
      ...
      factory: function() { return this.EQ(this.Release.IS_MOBILE, false); },
    },
    {
  ...
  methods: [
                ...
                for (let i = 0; i < dates.length; i++) {
                  ...
                  promises.push(
                      this.getLatestReleaseFromEachBrowserAtDate(date)
                          .then(releases => this.compute(numBrowsers, releases,
                                                         date)));
                }
    ...
    function compute(numBrowsers, releases, date) {
      ...
      // Compute from service, then store results in "browserMetricsDAO".
      return Promise.all(
          this.metricComputerTypes.map(type => this.metricComputerService.compute(
              type, releases, date).then(metrics => Promise.all(metrics.forEach(
