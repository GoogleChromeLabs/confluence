...
foam.CLASS({
  ...
  properties: [
    ...
    },
    {
      ...
      factory: function() {
        return [
          this.MetricComputerType.AGGRESSIVE_REMOVAL,
          this.MetricComputerType.BROWSER_SPECIFIC,
          this.MetricComputerType.FAILURE_TO_SHIP,
          this.MetricComputerType.API_VELOCITY,
        ];
      },
    },
    {
        ...
        for (let i = 0; i < numWorkers; i++) {
          ...
          workers[i] = this.stubFactory.get(this.BoxRegistry).create({
            delegate: this.ForkBox.create({
              critical: true,
              nodeParams:
              // i === 0 ? ['--inspect', '--debug-brk', '--max_old_space_size=4096'] : this.getForkNodeParams_(),
              this.getForkNodeParams_(),
  ...
  methods: [
      ...
      code: function() {
        return this.releaseDAO.orderBy(this.Release.RELEASE_DATE)
            ...
            .then(groups => Promise.all([
              ...
              this.computeApiVelocity(groups),
            ]));
      },
    },
    ...
    function computeApiVelocity(groups) {
      ...
      return Promise.all(groups.groupKeys.map(browserName => {
        ...
        return this.computeApiVelocity_(
    ...
    function computeBrowserMetrics_(numBrowsers, releases, date) {
      ...
      // ApiVelocity computed separately.
      const types = this.metricComputerTypes.filter(
          type => type !== this.MetricComputerType.API_VELOCITY);

      // Compute from service, then store results.
      return Promise.all(
          types.map(type => this.metricComputerService.compute(
              type, releases, date).then(metrics => Promise.all(metrics.map(
                  value => this.browserMetricsDAO.put(value))))));
    },
    function computeApiVelocity_(releases) {
      if (releases.length === 0) return undefined;
      const date = releases[0].releaseDate;
      const release = releases[0];
      this.info(`Computing API velocity: ${date}, ${release.id}`);

      // NEXT TODO: CONTINUE HERE: Trying to share code for MetricComputers
      // and ApiVelocity. This mostly implements ApiVelocity over the
      // Runner<=>Service bridge, but postprocessing (see Promise.all(...)
      // and end of above method) should be abstracted and run over all
      // service.compute(...) calls.

      var promises = [
        this.metricComputerService.compute([releases[0]],
                                           releases[0].releaseDate),
      ];
      for (var i = 0; i < releases.length - 1; i++) {
        const prev = releases[i];
        const next = releases[i + 1];
        promises.push(this.metricComputerService.compute([prev, next],
                                                         next.releaseDate));
      }
      return Promise.all(promises);
    },
