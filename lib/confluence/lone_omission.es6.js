  ...
  methods: [
    ...
    },
    {
      ...
      code: function(comparedReleases, prevReleases, date, release) {
        ...
        const otherBrowserProps = props
            .filter(p => comparedReleases
                .some(r => foam.util.equals(p.release, r)));
        const thisBrowserReleases = comparedReleases.concat([release]);
        const thisBrowserProps = props
            .filter(p => thisBrowserReleases
                .some(r => foam.util.equals(p.release, r)));
        ...
        return this.compatDAO.where(query).select().then(this.COUNT())
            .then(countSink => {
