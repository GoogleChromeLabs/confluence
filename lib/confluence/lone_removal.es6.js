  ...
  methods: [
    ...
    },
    {
      ...
      code: function(releases, date, release) {
        ...
        const otherReleaseIDs = releases.map((rls) => rls.releaseKey)
        ...
        return Promise.all([
          ...
          const currentOtherReleases = currentOtherAndPrevOwn[0].array;
          const prevOwnReleases = currentOtherAndPrevOwn[1].array;
    ...
    },
    {
      ...
      code: function(date, release, current, previous) {
        ...
        const prevLessOne = Array.from(previous);
        const postRemoval = prevLessOne.shift();

        const currentProps = current.map((r) => this.getPropertyForRelease(r));
        const prevProps = prevLessOne.map((r) => this.getPropertyForRelease(r));
        const postRemovalProp = this.getPropertyForRelease(postRemoval);
        const releaseProp = this.getPropertyForRelease(release);

        const E = this.ExpressionsSingleton.create();
        const query = E.AND(
            // Shipped in previous releases of this browser.
            E.OR(...prevProps.map((p) => E.EQ(p, true))),
            ...
            E.AND(...currentProps.map((p) => E.EQ(p, true))));
