...
foam.CLASS({
  ...
  methods: [
    function init() {
      // Run "onBeforeCompute" once, the first time "beforeCompute" is
      // published.
      this.sub('beforeCompute', foam.events.oneTime(this.onBeforeCompute));
    },
    {
      name: 'compute',
      returns: 'Promise',
      code: function(metricComputerType, releases, date) {
        this.pub('beforeCompute');
  ...
  listeners: [
    {
      ...
      documentation: `One-time listener to initialize container.`,
      code: function() {
        ...
        this.container.browserMetricsDAO = this.browserMetricsDAO;
      },
    }
  ]
});
