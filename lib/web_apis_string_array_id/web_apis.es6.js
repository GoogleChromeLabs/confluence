...
foam.CLASS({
  ...
  package: 'com.web.api',
  requires: [
    'com.web.api.Browser_StrId',
    'com.web.api.BrowserAPI_StrId',
    'com.web.api.WebInterface_StrId',
    'foam.dao.EasyDAO',
  ...
  methods: [
    {
      ...
      code: function(browserName, browserVersion,
        ...
        let browser = this.Browser_StrId.create({
        ...
        for (let i = 0; i < interfaceNames.length; i += 1) {
          ...
          for (let j = 0; j < apiCatalog[interfaceName].length; j += 1) {
            ...
            let webInterface = this.WebInterface_StrId.create({
              interfaceName,
              APIName,
            });
            let interfaceId = webInterface.id;
            this.interfaces.put(webInterface);
            this.browserAPIs.put(this.BrowserAPI_StrId.create({
              browserId,
              interfaceId,
    ...
    },
    {
      ...
      code: function(browserKey) {
        ...
        return this.Browser_StrId.create({
      ...
      code: function(browserAPIs, browserKeys) {
        return new Promise(function(resolve) {
          ...
          for (let i = 0; i < browserAPIs.length; i += 1) {
            ...
            let interfaceName = interfaceId[0];
            let apiName = interfaceId[1];
          ...
          Promise.all(promises).then(function() {
            console.timeEnd('stirng-array-id-process');
            resolve(apiMap);
          });
        }.bind(this));
      },
    },
    {
      ...
      code: function(browserKeys) {
        return new Promise(function(resolve) {
          ...
          console.time('string-array-id-query');
          this.browserAPIs
            .where(this.mlang.IN(this.BrowserAPI_StrId.BROWSER_ID, browserIds))
            .select()
            .then((arrayDao) => {
              console.timeEnd('string-array-id-query');
              console.time('stirng-array-id-process');
