  ...
  properties: [
    ...
    },
    {
      ...
      value: function f(gridRow) {
        const row = this.GridRow.create({
          id: gridRow.id && gridRow.id.clone ? gridRow.id.clone(this) :
              gridRow.id,
        });
        const allData = gridRow.data;
        const cols = this.cols;
        const data = new Array(cols.length);
...
foam.CLASS({
  ...
  methods: [
    function f(array) {
      ...
      const extracted = new Array(offsets.length);
...
foam.CLASS({
  ...
  methods: [
    ...
    function ARRAY_EXTRACT(...args) {
      return this.ArrayExtract.create({args});
    },
  ...
  methods: [
    ...
    },
    {
      ...
      documentation: `Return a foam.mlang.ExprProperty that transforms a GridRow
          ...
          "cols".`,
      code: function(...args) {
        const cols = this.getColIdxs_(args);
