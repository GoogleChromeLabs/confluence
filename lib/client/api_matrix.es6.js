...
foam.CLASS({
  ...
  classes: [
    {
      ...
      properties: [
        ...
        {
          name: 'delegateSub_',
          factory: function() { return foam.core.FObject.create(); },
        },
      ],

      methods: [
        function init() {
          this.delegateSub_.onDetach(() => this.cancelled = true);
        },
        function put(o) {
          if (this.cancelled) return;
          this.numResults++;
          this.delegate.put(o, this.delegateSub_);
        },
        function eof() {},
      ],
    },
  ...
  methods: [
    function select_(ctx, sink, skip, limit, order, predicate) {
      return this.selectBatch_(ctx, sink, skip, limit, order, predicate);
    },
    function selectBatch_(ctx, sink, skip, limit, order, predicate) {
      const hasSkip = foam.Number.isInstance(skip);
      const hasLimit = foam.Number.isInstance(limit);
      if (hasLimit && limit <= 0) {
        sink.eof && sink.eof();
        return sink;
      }

      const batchLimit = hasLimit ? Math.min(limit, this.batchSize) :
            this.batchSize;
      return this.delegate.select_(ctx, this.BatchSink.create({delegate: sink}),
                                   skip, batchLimit, order, predicate)
          .then(batchSink => {
            if (batchSink.cancelled || batchSink.numResults === 0) {
              sink.eof && sink.eof();
              return sink;
            }

            const nextSkip = hasSkip ? skip + batchSink.numResults :
                  batchSink.numResults;
            const nextLimit = hasLimit ? limit - batchSink.numResults : limit;
            return this.selectBatch_(ctx, sink, nextSkip, nextLimit, order,
                                     predicate);
          });
    },
  ],
...
foam.CLASS({
  ...
  requires: [
    ...
    'foam.dao.ArraySink',
    'org.chromium.apis.web.BatchDAO',
  ...
  properties: [
    {
      ...
      factory: function() {
        return this.BatchDAO.create({
          delegate: this.releaseWebInterfaceJunctionDAO,
        });
      },
    },
    {
    ...
    },
    {
      ...
      code: function(metadata) {
        ...
        return verifyReleaseKeys().then(maybe(() => {
          ...
          let promises = [];
          return this.junctionDAO.where(query)
              .select(this.AnonymousSink.create({
                sink: {
                  put: (junction, sub) => {
                    if (cancelled) sub.detach();
                    promises.push(
                        maybe(this.webInterfaceDAO.find.bind(
                            this.webInterfaceDAO, junction.targetId))()
                          .then(maybe(iface => {
              ...
              })).then(maybe(() => Promise.all(promises)))
      ...
      code: function(id) {
        return this.getLatestReleases()
            .then(latest => this.setReleaseKeys(
                latest.map(release => release.releaseKey)));
      },
    },
    {
      name: 'getMatrixCSV',
