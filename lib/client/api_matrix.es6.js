...
foam.CLASS({
  ...
  // TODO(markdittmer): Really "static methods". Update once
  // https://github.com/foam-framework/foam2/issues/613 is fixed.
  constants: {
    // Separate preferred latest release of each browser from the rest of
    // releases in injected "releaseDAO". Both collections are returned in
    // descending release date order.
    SEPARATE_LATEST_RELEASES: function(releaseDAO) {
      const pkg = org.chromium.apis.web;
      const E = foam.mlang.ExpressionsSingleton.create();
      return releaseDAO.orderBy(E.DESC(pkg.VersionedRelease.RELEASE_DATE))
          .select().then(sink => {
            const array = sink.array;

            let firstReleaseMap = {};
            let firstReleases = [];
            let restReleases = Array.from(array);

            // Next, load API data from latest release of each browser.
            for (let i = 0; i < restReleases.length; i++) {
              const release = restReleases[i];
              if (!firstReleaseMap[release.browserName] &&
                  (release.osName === 'Windows' ||
                   release.browserName === 'Safari')) {
                firstReleases.push(release);
                firstReleaseMap[release.browserName] = true;
                foam.Array.remove(restReleases, release);
                i--;
              }
            }

            return {latest: firstReleases, rest: restReleases};
          });
    },
  },
