...
foam.CLASS({
  ...
  documentation: `Metadata associated with latest toMatrix() request from view
      identified by "id".`,
  ...
  properties: [
    {
      class: 'String',
      name: 'id',
      documentation: `An opaque identifier for the view that requested a matrix
    ...
    },
    {
      ...
      of: 'String',
      name: 'releaseKeys',
      documentation: `The "releaseKey" (and "id") values of the releases for the
          requested matrix.`,
    },
    {
      class: 'String',
      name: 'searchKey',
      documentation: `The interface/API name query filtering the requested
          matrix.`,
    },
    {
      name: 'releaseOptions',
      documentation: `Optional map of {<releaseKey>: Boolean}. When set, it is
          used to filter out all APIs that do not match all key/value pairs.`,
      value: null,
    },
    {
      name: 'numAvailable',
      documentation: `Optional Int or Array<Int>. When set, it is used to filter
          out APIs that are available on greater or fewer of the releases
          identified by "releaseKeys".`,
      value: null,
    },
  ],
});
...
foam.CLASS({
  ...
  documentation: `Latest <metadata request> / <data response> pair for
      toMatrix() requests from view identified by "id".`,
  ...
  properties: [
    {
      ...
      name: 'metadata',
      documentation: `Latest (per view) metadata provided via a toMatrix()
          request.`,
      required: true,
    },
    {
      name: 'matrix',
      documentation: 'Latest (per view) matrix returned from toMatrix().',
  ...
  properties: [
    {
      ...
      documentation: `A map of {<MatrixMetadata.id>: <detachable>}. Used to
          detach() from (i.e., cancel) obsolete toMatrix() requests.`,
      factory: function() { return {}; },
    },
  ],
  ...
  methods: [
    ...
    },
    {
      ...
      args: [
        ...
        {
          name: 'numAvailable',
          documentation: `Optional Int or Array<Int> from
              MatrixMetadata.numAvailble.`,
        },
        {
          name: 'releaseOptions',
          typeName: 'JSON',
          documentation: `Optional JSON From MatrixMetadata.releaseOptions.`,
        },
      ],
      returns: {
        typeName: 'Boolean',
        ...
        for (let i = 0; i < interfaces.length; i++) {
          ...
          for (let j = 0; j < APIs.length; j++) {
            ...
            for (let k = 0; k < releaseKeys.length; k++) {
              const foundInRelease = matrix[interfaceName][apiName]
                  .hasOwnProperty(releaseKeys[k]);
              foundAPI = foundAPI || foundInRelease;
              row.push(foundInRelease);
            }
            if (foundAPI) table.push(row);
          }
        }
        let csv = '';
