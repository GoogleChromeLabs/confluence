...
foam.CLASS({
  ...
  methods: [
    ...
    function selectBatch_(ctx, sink, skip, limit, order, predicate) {
      ...
      const batchSkip = skip;
      const batchLimit = Math.min(limit, this.batchSize);
      return this.delegate.select_(ctx, this.BatchSink.create({delegate: sink}),
                                   batchSkip, batchLimit, order, predicate)
          .then(batchSink => {
            ...
            return this.selectBatch_(ctx, sink, skip + batchSink.numResults,
                                     limit - batchSink.numResults, order,
                                     predicate);
          });
    },
  ],
  ...
  methods: [
    ...
    },
    {
      ...
      code: function(metadata) {
        ...
        return this.verifyReleaseKeys_(releaseKeys).then(maybe(() => {
  ...
  methods: [
    ...
    },
    {
      ...
      code: function(id, releaseKeys) {
        ...
        // TODO(markdittmer): These should probably be verified.
        return releaseKeys;
      },
    },
