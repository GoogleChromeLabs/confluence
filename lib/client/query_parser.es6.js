  ...
  methods: [
    function interpretKeyValue(key, value, parser) {
      ...
      if (releases.count === 1) {
        ...
        return this.EQ(this.gridDAO.extractCols.apply(this.gridDAO, bools),
                       bools);
      }
    },
    function interpretKeyword(kw, parser) {
    ...
    function getMatchingReleases(key) {
      ...
      const browserVersion = match[2].toLowerCase();
      const osName = match[3].toLowerCase();
      const osVersion = match[4].toLowerCase();
