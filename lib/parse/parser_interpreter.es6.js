  ...
  grammars: [
    {
      ...
      actions: [
        ...
        function or(exprs) {
          ...
          return this.OR(...exprs);
        },
        function and(exprs) {
          if (exprs.length === 0) return this.TRUE;
          if (exprs.length === 1) return exprs[0];
          return this.AND(...exprs);
        },
        function countExpr(countValue) {
          const value = parseInt(countValue[1]);
          const cols = this.selected
              .filter((col) => this.CompatProperty.isInstance(col));
          return this.EQ(this.ARRAY_COUNT(this.SEQ(...cols),
              this.TRUTHY()),
          value);
        },
        function inExpr(inAndValues) {
          ...
          return cols.length === 1 ? this.EQ(cols[0], true) :
              this.AND(...cols.map((col) => this.EQ(col, true)));
        },
        function notinExpr(notinAndValues) {
          ...
          return cols.length === 1 ? this.EQ(cols[0], false) :
              this.AND(...cols.map((col) => this.EQ(col, false)));
        },
        function keyword(kw) {
...
foam.LIB({
  name: 'org.chromium.parse.util',

  methods: [
    {
      name: 'getTerseReleaseId',
      documentation: `id is String of the form:
          BrowserName_bows.er.ver.sion_OSName_os.ver.sion

          og.chromium.parse.QueryParser will prefix match
          <name><version><name><version> sequence using transition between
          alphabetical+punctuation and numeric+punctuation.`,
      args: [
        {
          name: 'id',
          documentation: 'An org.chromium.apis.web.Release.ID value.',
          typeName: 'String',
        },
      ],
      returns: 'String',
      code: function getTerseReleaseId(id) {
