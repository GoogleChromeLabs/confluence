      ...
      this.view.data$.mapTo(
          ...
          (queryStr) => {
            ...
            try {
              ...
              if (parse.rest.length === 0) {
                ret = parse.result;
              } else {
                this.reportError_(
                    `Query parser error: Failed to parse "${parse.rest}"`);
              }
            } catch (e) {
              this.reportError_('Query parser error: ', e);
            }
            return ret;
          });
      this.SUPER();
    },
    function reportError_(str) {
      // When Materialize is installed, show a toast to surface the error to the
      // user. Otherwise, delegate to error logging function.
      if (this.window.Materialize) {
        this.window.Materialize.toast(str, 4000);
      } else {
        this.error(str);
      }
    },
