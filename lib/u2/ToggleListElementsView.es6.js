  ...
  classes: [
    {
      ...
      methods: [
        ...
        function addElement() {
          ...
          // Operate on array copy to affect meanginful set of bound property:
          // this.selected.
          const ret = Array.from(ed);
        ...
        function removeElement() {
          const elem = this.selectable[this.selectableIdx];
          const ed = this.selected;
          const idx = ed.findIndex((edElem) => edElem.name == elem.name);
          if (idx >= 0) {
            const ret = Array.from(ed);
            ret.splice(idx, 1);
            this.selected = ret;
