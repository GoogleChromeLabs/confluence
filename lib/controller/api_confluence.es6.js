...
angular.module('confluence')
  .controller('confluenceController', ['$scope', 'api', function($scope, api) {
    ...
    $scope.newAggressiveRemovalView = function(browserOneYearAgo,
        prevReleaseBrowsers, currBrowsers) {
      let browsers = [browserOneYearAgo].concat(
          prevReleaseBrowsers, currBrowsers);
      let browserOptions = {};
      // Set browser options to be false for browserOneYearAgo and
      // true for all currBrowsers. And set numAvailable to be 1 +
      // currBrowsers.length to browsers.length. So it returns
      // APIs that not exists in browserOneYearAgo but avaiable
      // in all current Browsers and some prevReleaseBrowsers;
      browserOptions[browserOneYearAgo.browserKey] = false;
