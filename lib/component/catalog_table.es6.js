...
function catalogTableController($scope, api) {
  ...
  const ctlr = this;
  const matrixCtlr = api.apiMatrixController;
  ...
  ctlr.$onChanges = function(changes) {
    console.log('ctlr.$onChanges(changes)', changes);
    if (changes.releases &&
        !foam.util.equals(changes.releases.previousValue,
                          changes.releases.currentValue)) {
      matrixCtlr.setReleases(changes.releases.currentValue);
    }
    if (changes.releaseOptions || changes.numAvailable) {
      matrixCtlr.setOpts({
        searchKey: $scope.searchKey,
        releaseOptions: changes.releaseOptions ?
            changes.releaseOptions.currentValue : ctlr.releaseOptions,
        numAvailable: changes.numAvailable ?
            changes.numAvailable.currentValue : ctlr.numAvailable,
      });
    }
  };
  ...
  $scope.downloadCSV = function() {
    ...
    let releaseKeys = getReleaseKeys(ctlr.releases);
    matrixCtlr.getMatrixCSV(releaseKeys).then(csv => {
      if (csv === null) return;
  ...
  $scope.search = function($event) {
    ...
    matrixCtlr.setOpts({
      searchKey: $scope.searchKey,
      releaseOptions: ctlr.releaseOptions,
      numAvailable: ctlr.numAvailable,
    });
  };
}
