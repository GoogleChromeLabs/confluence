...
require('../cloudstore/rate_limiter.es6.js');
...
foam.CLASS({
  ...
  properties: [
    ...
    {
      class: 'Boolean',
      name: 'toCloudDatastore',
      documentation: `An optional boolean property. When set true,
        the importer will import webInterface and browserAPIs to
        DAO with a rate limit.`,
      values: false,
    },
    {
      name: 'limiter',
      documentation: `An RateLimiter object to constraint the number
        of put per minute when toCloudDatastore is set to true.`,
      factory: function() {
        return this.RateLimiter.create({interval: 10});
      },
    },
  ],
  methods: [
        ...
        osName, osVersion, apiCatalog) {
          ...
          for (let i = 0; i < interfaceNames.length; i += 1) {
            ...
            for (let j = 0; j < apiCatalog[interfaceName].length; j += 1) {
              ...
              if (this.toCloudDatastore) {
                this.limiter.enqueue(() => this.interfaceDAO.put(webInterface));
                this.limiter.enqueue(() => webInterface.browsers.put(browser));
              } else {
