  ...
  methods: [
    {
      ...
      code: function(browserName, browserVersion,
        ...
        const promises = [];
        const release = this.Release.create({
          browserName,
          browserVersion,
          osName,
          osVersion,
        ...
        const interfaceNames = Object.keys(apiCatalog);
        for (let i = 0; i < interfaceNames.length; i++) {
          const interfaceName = interfaceNames[i];
          for (let j = 0; j < apiCatalog[interfaceName].length; j++) {
            const apiName = apiCatalog[interfaceName][j];
            const webInterface = this.WebInterface.create({
              interfaceName,
              apiName,
            });
